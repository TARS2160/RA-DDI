...
def train_improved_ddi_model(ddi_source="PDD Graph"):
    ...
    # 2. 加载预训练的药物嵌入
    print("=== 加载预训练药物嵌入 ===")
    data = torch.load("models/drug_global_embeddings.pt")
    ...
    # 定义数据库路径和规格
    DB_INFO = {
        "ogbl-ddi": {"path": "ogbl_ddi/test1.csv", "min_samples": 1000},
        "DDInter": {
            "path": "ddinter/test3.csv",
            "min_samples": 100,
            "label_map": {"Moderate": 0, "Minor": 1, "Major": 2}
        },
        "PDD Graph": {"path": "PDD_graph/test2.csv", "min_samples": 10}
    }
...
def evaluate_embedding_quality():
    """评估嵌入质量"""
    print("\n=== 嵌入质量评估 ===")
    data = torch.load("models/model/drug_global_embeddings.pt")
