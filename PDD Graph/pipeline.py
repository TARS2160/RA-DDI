...
'''
这是一个完整的数据处理 pipeline，一步跑完生成以下内容：
patient→drug dataframe：每行一条患者-药物关系
drug-drug 共现表：统计每个患者服用的药物组合，得到 drug pair 及其共现次数（weight）
drug-disease 表：患者-疾病关系展开，生成 drug-disease 对
映射覆盖率与统计报告：统计患者数、药物数、疾病数、映射到 DrugBank ID 的覆盖率等
在 Step 6 新增了 ddi_df，生成正负样本二分类数据。
保存路径直接放到 data/PDD_graph/test2.csv，方便 part_four.py 调用。
负样本比例默认 1:1（你也可以改 num_negatives 为 2×正样本等
'''