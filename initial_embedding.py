...
class SimpleDrugEncoder(nn.Module):
    def __init__(self, hidden_dim=256, pretrained_model_path=None, smiles_vocab_size=200):
        ...
        if pretrained_model_path is None:
            pretrained_model_path = "huggingface/biobert_v1.1_pubmed" #现有的预训练模型，可以从huggingface获取
...
def optimized_training(dataset, pos_pairs, drug_ids, epochs=5, batch_size=8, lr=1e-4):
    ...
    }, "model/drug_initial_embeddings.pt")  #"models/model/drug_initial_embeddings.pt"
...
if __name__ == "__main__":
    ...
    # 1) 读取SMILES
    print("加载SMILES数据...")
    smiles_df = pd.read_csv("drugbank/drug_smile_filtered.csv")
    ...
    # 2) 文本tokenize
    print("处理文本数据...")
    core_df = pd.read_csv("drugbank/drugbank_core_data.csv")
    desc_list = []
    for did in drug_ids:
    ...
    tokenizer = AutoTokenizer.from_pretrained("huggingface/biobert_v1.1_pubmed")
    ...
    # 3) 读取KG属性
    print("处理KG数据...")
    kg_df = pd.read_csv("drugbank/drugbank_kg_triples_cleaned.csv")
    rel_vocab = {}
    kg_list = []
